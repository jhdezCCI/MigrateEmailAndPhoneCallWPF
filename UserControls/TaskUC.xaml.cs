...
namespace MigrateEmailAndPhoneCallWPF.UserControls
{
    ...
    public partial class TaskUC : UserControl
    {
            ...
            await this.Dispatcher.Invoke(async () =>
             {
                 try
                 {
                     ...
                     if (xSource == "InsideSales")
                         dgTask.ItemsSource = await ProcessPhonecallIS();
                     else
                         dgTask.ItemsSource = await ProcessTask();


                     tbComment.Text += $"{Environment.NewLine}Loading Source Finished at {DateTime.Now}";
                     tbTotalRecords.Text = (dgTask.ItemsSource as List<TaskCRM>).Count.ToString();
        ...
        private async Task<List<TaskCRM>> ProcessTask()
        {
            xCancelTokenSource = new CancellationTokenSource();
            await Task.Run(() =>
            {
                List<TaskRecord> pcList;
                ...
                var crTask = new ConcurrentBag<TaskCRM>();
                ...
                Parallel.ForEach(pcList, new ParallelOptions() { MaxDegreeOfParallelism = 500 },
                    (item, loopState, index) =>
                    {
                        ...
                        {
                            temp.DueDate = item.DueDate.Value;
                        }
        ...
        private async Task<List<TaskCRM>> ProcessPhonecallIS()
        {
            xCancelTokenSource = new CancellationTokenSource();
            await Task.Run(() =>
            {
                ...
                List<Entity> pcIS = tcrm.GetTaskIS(filterGuid, xFromDt);
                tcrm.EndConnection();
                ...
                foreach (var item in pcIS)
                {
                    ...
                    {
                        temp.DueDate = DateTime.Parse(item.Attributes["scheduledend"].ToString());
                    }
                ...
                //if (rmi.Count > 0)
                //{
                //    string path = $"C:\\Users\\jxh0tw1\\Documents\\Migration\\activity\\Task{xSource}{DateTime.Now.Year}{DateTime.Now.Month}{DateTime.Now.Day}_{DateTime.Now.Hour}_{DateTime.Now.Minute}.csv";
        ...
        private async void BtMigrate_Click(object sender, RoutedEventArgs e)
        {
            ...
            int countPB = 0;
            pbar.Value = 0;
            pbar.Maximum = (dgTask.ItemsSource as List<TaskCRM>).Count;
            ...
            if (crm.StartConnection(xEnv.ToLower(), true))
            {
                await Task.Run(() =>
                {
                    ...
                    ConcurrentBag<RecordMissingInfo> missingRegarding) = crm.UpsertTaskRecords(crm.conn, (dgTask.ItemsSource as List<TaskCRM>), 300);

                    tbComment.Dispatcher.Invoke(() => tbComment.Text += $"{processResult.Count} Tasks Success" + Environment.NewLine, System.Windows.Threading.DispatcherPriority.Background);

                    tbComment.Dispatcher.Invoke(() => tbComment.Text += $"{missingRegarding.Count} Tasks do not have Regarding" + Environment.NewLine, System.Windows.Threading.DispatcherPriority.Background);

                    tbComment.Dispatcher.Invoke(() => tbComment.Text += $"{processE.Count} Tasks Failure" + Environment.NewLine, System.Windows.Threading.DispatcherPriority.Background);
        ...
        private List<TaskRecord> GetFiberTask()
        {
            ...
            List<TaskRecord> pcList = new List<TaskRecord>();

            if (filterGuid.Length > 0)
            {
                pcList = (from a in fiberctx.Tasks
        ...
        private List<ActivityPartyRecord> GetFiberActivityParty()
        {
            ...
            if (xFromDt.Year != 1)
            {
                apList = (from a in fiberctx.ActivityParties
                          ...
                          join b in fiberctx.Tasks on a.ActivityId equals b.ActivityId
                          where a.ActivityTypeCode == 4212 && b.ModifiedOn >= xFromDt &&
            ...
            else
            {
                apList = (from a in fiberctx.ActivityParties
                          join r in fiberctx.EntityViews on a.PartyObjectTypeCode equals r.ObjectTypeCode
                          join b in fiberctx.Tasks on a.ActivityId equals b.ActivityId
                          where a.ActivityTypeCode == 4212 && b.CreatedOn >= DateTime.Parse("2019-01-01") &&
                          (b.RegardingObjectTypeCode == 1 || b.RegardingObjectTypeCode == 2 ||
                           b.RegardingObjectTypeCode == 3 || b.RegardingObjectTypeCode == 4)
                          select new ActivityPartyRecord
                          {
        ...
        private List<TaskRecord> GetWirelessTask()
        {
        ...
        private List<ActivityPartyRecord> GetWirelessActivityParty()
        {
            ...
            if (xFromDt.Year != 1)
            {
                apList = (from a in wctx.ActivityParties
                          ...
                          join b in wctx.Tasks on a.ActivityId equals b.ActivityId
                          where a.ActivityTypeCode == 4212 && b.ModifiedOn >= xFromDt &&
                          (b.RegardingObjectTypeCode == 1 || b.RegardingObjectTypeCode == 2 ||
